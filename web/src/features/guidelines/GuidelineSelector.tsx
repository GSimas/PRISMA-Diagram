    ...
    <main id="main-content" className="content-page guidelines-page">
      ...
      <div className="guideline-layout">
        <form className="guideline-form">
          ...
          <fieldset><legend>2. Qual é o tipo principal?</legend><select value={type} onChange={(event) => setType(event.target.value as typeof type)}><option value="systematic">Revisão sistemática</option><option value="scoping">Revisão de escopo</option><option value="living">Revisão sistemática viva</option><option value="network">Metanálise em rede</option><option value="integrative">Revisão integrativa</option></select></fieldset>
        ...
        <aside className="recommendation-panel" aria-live="polite"><p className="kicker">ORIENTAÇÃO INICIAL</p><h2>{recommendations.join(' + ')}</h2><p>{stage === 'protocol' ? 'PRISMA-P é específico para o desenvolvimento e relato de protocolos. Não é sinônimo de PRISMA 2020.' : stage === 'complete' && type === 'integrative' ? 'Revisões integrativas não têm uma extensão oficial do PRISMA. O PRISMA 2020 é sugerido apenas como referência estrutural para o diagrama de fluxo e o relato; o desenho metodológico deve seguir uma abordagem própria (ex.: Whittemore & Knafl).' : 'A diretriz principal ou extensão deve ser lida junto com seus documentos explicativos oficiais.'}</p><ul>{recommendations.map((name) => <li key={name}>{name}</li>)}</ul>{stage === 'complete' && <a className="primary-button" href={`/builder?model=${model}`}>Iniciar modelo recomendado <ArrowRight size={16} /></a>}<small>Limite: este assistente usa respostas gerais e não avalia o desenho completo, a área temática ou exigências editoriais.</small></aside>
      </div>
