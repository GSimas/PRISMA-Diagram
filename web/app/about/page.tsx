...
export const metadata: Metadata = { title: 'Sobre', description: 'Sobre o PRISMA Lab, seus objetivos, público e independência.', alternates: { canonical: '/about' } };
const faq = [
...
export default function AboutPage() { return <InfoPage eyebrow="PROJETO AUTORAL · CIÊNCIA ABERTA" title="Sobre o PRISMA Lab" lead="Uma ferramenta independente para transformar contagens e decisões documentadas em relatos mais claros, rastreáveis e publicáveis."><h2>Propósito</h2><p>O PRISMA Lab atende pesquisadores, estudantes, bibliotecários, orientadores, revisores e editores envolvidos em síntese de evidências. Ele combina educação, edição vetorial, validação numérica, checklist e exportação científica.</p><h2>Princípios</h2><ul><li>Dados locais e portáteis.</li><li>Regras determinísticas e explicáveis.</li><li>Sem correções silenciosas de dados científicos.</li><li>Acessibilidade e internacionalização desde o modelo de domínio.</li><li>Independência institucional explícita.</li></ul><h2>Perguntas frequentes</h2>{faq.map(([question, answer]) => <details key={question}><summary>{question}</summary><p>{answer}</p></details>)}</InfoPage>; }
